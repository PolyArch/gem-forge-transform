...
void SIMDPass::simdTransform(DynamicInnerMostLoop &DynamicLoop) {
   ...
   */
  {
    ...
    FixRegDependence(this->Trace->RegDeps.at(
        this->Trace->DynamicInstructionList.back()->getId()));
  ...
  for (auto NewDynamicInst : DynamicLoop.NewDynamicInstList) {
    ...
    this->Trace->AliveDynamicInstsMap.emplace(NewDynamicInst->getId(),
                                              NewDynamicInstIter);
...
SIMDPass::createDynamicInstsForScatterLoad(llvm::LoadInst *StaticLoad,
  ...
  for (auto DynamicId : DynamicIds) {
    ...
    NewDynamicIds.push_back(NewDynamicInst->getId());
  ...
  auto &NewPackRegDeps =
      this->Trace->RegDeps
          .emplace(std::piecewise_construct,
                   std::forward_as_tuple(NewDynamicPackInst->getId()),
                   std::forward_as_tuple())
          .first->second;
  auto &NewPackMemDeps =
      this->Trace->MemDeps
...
DynamicInnerMostLoop::DynamicInnerMostLoop(StaticInnerMostLoop *_StaticLoop,
  ...
  for (auto Iter = _Begin; Iter != _End; ++Iter) {
    ...
    DynamicIdVec[CurrentIter - 1] = (*Iter)->getId();
  }
