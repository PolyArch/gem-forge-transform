...
$(KERN): $(KERN).bc input.data
	clang -O0 $< -o $@ $(LINK_FLAG)

# Instrument
$(KERN)_trace.ll: $(KERN).bc
	opt $(OPT_ARGS) -debug-only=TracePass -trace-pass -trace-function=run_benchmark -o $@ $^

$(KERN)_replay.ll: $(KERN).bc
	opt $(OPT_ARGS) -debug-only=ReplayPass,DataGraph -replay -trace-file=$(TRACER_FILE_NAME) -trace-format=$(TRACE_FILE_FORMAT) $^ -o $@ 

$(KERN)_cca.ll: $(KERN).bc
	opt $(OPT_ARGS) -debug-only=ReplayPass,DataGraph -replay-cca -trace-file=$(TRACER_FILE_NAME) -cca-max-fus=$(CCA_MAX_FUS) $^ -o $@ 

$(KERN)_cca_analysis.ll : $(KERN).bc
	opt $(OPT_ARGS) -debug-only=ReplayPass,DataGraph -stream-analysis-trace -stream-trace-file=$(TRACER_FILE_NAME) -analyze $^
...
$(KERN)_replay: $(KERN)_replay.ll ${ROOT_DIR}/benchmark/replay.c
	clang -c -O0 $(KERN)_replay.ll -o $(KERN)_replay_before_gcc.o
	# Must use gcc to link gem5 pseudo instruction asm.
	gcc -O0 -I${GEM5_DIR}/include ${ROOT_DIR}/benchmark/replay.c ${GEM5_DIR}/util/m5/m5op_x86.S $(KERN)_replay_before_gcc.o -o $@ ${LINK_FLAG}

$(KERN)_cca: $(KERN)_cca.ll ${ROOT_DIR}/benchmark/replay.c
...
clean_replay:
	...
	rm -f $(KERN)_replay_before_gcc.o

clean_all: clean clean_trace clean_replay
	rm -f *.txt *.gz *.asm *.data llvm_trace
	rm -rf results*
	rm -rf DerivO3CPU*