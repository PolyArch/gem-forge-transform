...
__attribute__((noinline)) void kernel(char SEQA[ALEN], char SEQB[BLEN],
...
fill_out:
  ...
  // TraceBack (n.b. aligned sequences are backwards to avoid string
  // appending)
  a_idx = ALEN;
  b_idx = BLEN;
  a_str_idx = 0;
...
void needwun(char SEQA[ALEN], char SEQB[BLEN], char alignedA[ALEN + BLEN],
  ...
  for (int tdg_i = 0; tdg_i < TDG_WORKLOAD_SIZE; ++tdg_i) {
    kernel(SEQA, SEQB, alignedA, alignedB, M, ptr);
  }
}
