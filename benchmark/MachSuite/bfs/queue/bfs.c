...
__attribute__((noinline)) void bfs_kernel(node_t nodes[N_NODES],
...
init_levels:
  for (n = 0; n < N_NODES; n++) level[n] = MAX_LEVEL;
init_horizons:
  for (level_t i = 0; i < N_LEVELS; i++) level_counts[i] = 0;
  q_in = 1;
  q_out = 0;
...
void bfs(node_t nodes[N_NODES], edge_t edges[N_EDGES],
  ...
  for (int tdg_i = 0; tdg_i < TDG_WORKLOAD_SIZE; ++tdg_i) {
    bfs_kernel(nodes, edges, starting_node, level, level_counts);
  }
}
