...
__attribute__((noinline)) void kernel(
    TYPE force_x[nAtoms], TYPE force_y[nAtoms], TYPE force_z[nAtoms],
...
void md_kernel(TYPE force_x[nAtoms], TYPE force_y[nAtoms], TYPE force_z[nAtoms],
  ...
  for (int tdg_i = 0; tdg_i < TDG_WORKLOAD_SIZE; ++tdg_i) {
    kernel(force_x, force_y, force_z, position_x, position_y, position_z, NL);
  }
}
