...
#pragma clang loop vectorize(disable) unroll(disable) interleave(disable)
  ...
  gf_stream_nuca_region("gfm.conv3d.i", I, sizeof(I[0]), Ni, Nx, Ny);
  gf_stream_nuca_region("gfm.conv3d.o", O, sizeof(O[0]), Nn, Nx, Ny);
  gf_stream_nuca_region("gfm.conv3d.k", K, sizeof(K[0]), Ni * Kx * Ky * Nn);
  gf_stream_nuca_align(I, I, 1);
  gf_stream_nuca_align(I, I, Ni);
  gf_stream_nuca_align(I, I, Nx * Ni);
  gf_stream_nuca_align(O, O, 1);
  gf_stream_nuca_align(O, O, Nn);
  gf_stream_nuca_align(O, O, Nx * Nn);
  gf_stream_nuca_remap();

  gf_detail_sim_start();

  if (warm) {
    gf_warm_array("gfm.conv3d.i", I, sizeof(I[0]) * Ni * Nx * Ny);
    gf_warm_array("gfm.conv3d.o", O, sizeof(O[0]) * Nn * Nx * Ny);
    gf_warm_array("gfm.conv3d.k", K, sizeof(K[0]) * Ni * Kx * Ky * Nn);
  }
