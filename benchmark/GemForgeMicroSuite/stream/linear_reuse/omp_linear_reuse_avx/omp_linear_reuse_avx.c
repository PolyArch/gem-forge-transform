...
__attribute__((noinline)) Value foo(Value *a, Value *b, Value *c, int N) {
  ...
  const int64_t reuseSize = 8;
  const int64_t reuseCount = 2;
...
#endif
  for (int64_t i = 0; i < N; i += 16 * reuseSize) {
#pragma clang loop unroll(disable) vectorize(disable)
    for (int64_t j = 0; j < reuseCount; ++j) {
#pragma clang loop unroll(disable) vectorize(disable)
...
#ifdef GEM_FORGE
  // m5_stream_nuca_region("gfm.linear_reuse.a", a, sizeof(a[0]), N);
  // m5_stream_nuca_region("gfm.linear_reuse.b", b, sizeof(b[0]), N);
  // m5_stream_nuca_region("gfm.linear_reuse.c", c, sizeof(c[0]), N);
