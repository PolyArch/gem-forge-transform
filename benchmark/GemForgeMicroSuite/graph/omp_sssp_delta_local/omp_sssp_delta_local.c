...
#define getLocalBucket(idx) (localBuckets + ((idx) % numBuckets))
...
#pragma omp parallel shared(currentBucketIdx, iter) firstprivate(              \
    nodes, edges, numNodes, vs, globalBuckets, globalUpdateBuffer)
  {
    ...
    Bucket *localBuckets = threadLocalBuckets[tid];

    while (1) {

      // Get the current global bucket.
      Bucket *currentGlobalBucket = getGlobalBucket(currentBucketIdx);
      Bucket *currentLocalBucket = getLocalBucket(currentBucketIdx);
...
#pragma clang loop unroll(disable) interleave(disable)
          for (uint64_t j = start; j < end; ++j) {
            ...
            if (newBucketIdx < oldBucketIdx ||
              ...
              Bucket *newBucket = getLocalBucket(newBucketIdx);
#ifndef GEM_FORGE
              ...
              uint64_t newBucketEnqueueIdx = newBucket->size++;
      ...
      for (int i = 0; i < numBuckets; ++i) {
        Bucket *localBucket = getLocalBucket(currentBucketIdx + i);
        uint64_t localBucketSize = localBucket->size;
        Bucket *globalBucket = (i == 0) ? globalUpdateBuffer
                                        : getGlobalBucket(currentBucketIdx + i);
        // Clear the size.
        localBucket->size = 0;
        if (localBucketSize > 0) {
          // Write to global queue.
          const uint64_t pos = __atomic_fetch_add(
              &globalBucket->size, localBucketSize, __ATOMIC_RELAXED);
#ifndef GEM_FORGE
          // Well this implementation will likely blow up the queue.
          assert(pos + localBucketSize < numNodes);
#endif
          __builtin_memcpy(&globalBucket->data[pos], localBucket->data,
                           localBucketSize * sizeof(localBucket->data[0]));
        }
      }
...
int main(int argc, char *argv[]) {
  ...
  Bucket **threadLocalBuckets =
      aligned_alloc(CACHE_LINE_SIZE, sizeof(Bucket *) * numThreads);
  for (int i = 0; i < numThreads; ++i) {
    threadLocalBuckets[i] =
        aligned_alloc(CACHE_LINE_SIZE, sizeof(Bucket) * numBuckets);
    for (int j = 0; j < numBuckets; ++j) {
      threadLocalBuckets[i][j].size = 0;
      threadLocalBuckets[i][j].data =
          malloc(sizeof(GraphIndexT) * graph.numNodes);
    }
  }
  // Push the root.
...
#pragma omp parallel for schedule(static)
  ...
  volatile int ret = foo(&graph, values, globalBuckets, threadLocalBuckets,
                         &globalUpdateBuffer, numBuckets, delta, 10000000);
#ifdef GEM_FORGE
  m5_detail_sim_end();
  exit(0);
#else
