  ...
  void testInstDependence(
    ...
    while (Reader.read(Inst)) {
      ...
      for (const auto &Dep : Inst.deps()) {
        ...
        ASSERT_TRUE(DynamicIdToSequenceMap.count(Dep.dependent_id()) != 0)
            << "Failed to translate the dynamic id " << Dep.dependent_id();
        ActualDeps.insert(DynamicIdToSequenceMap.at(Dep.dependent_id()));
      }
